    ...
    for commit in commits:
        ...
        for node in commit['nodes']:
            if (node['type'] == 'file'):
                ...
                graph += f'  {counter}[label="Directory: {node["name"]}", color="{dirColor}", shape=component, style="rounded,filled"]\n'
                counter += 1
                ...
                for n in node['nodes']:
                    if (n['type'] == 'file'):
                        ...
                        graph += f'  {counter}[label="Directory: {n["name"]}", color="{dirColor}", shape=component, style="rounded,filled"]\n'
                        counter += 1
                        ...
                        for k in n['nodes']:
                            if (k['type'] == 'file'):
                                ...
                                graph += f'  {counter}[label="Directory: {k["name"]}", color="{dirColor}", shape=component, style="rounded,filled"]\n'
                                counter += 1
    counter = 0
    commitCounter = 0
    dirCounter = 0
